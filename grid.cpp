...
void Grid::add(Player&p){
	...
	if(p.weapon)if(c[p.x+dx[p.dir]][p.y+dy[p.dir]]==6)player.money+=10;
	if(c[p.x][p.y]==11)++countKey;
	if(p.weapon)if(c[p.x+dx[p.dir]][p.y+dy[p.dir]]==11)++countKey;
	if(c[p.x][p.y]==10 or (p.weapon and c[p.x+dx[p.dir]][p.y+dy[p.dir]]==10))touchPalace=1;
	if(c[p.x][p.y]==12 or (p.weapon and c[p.x+dx[p.dir]][p.y+dy[p.dir]]==12))p.hp=max(0,p.hp-1);
	c[p.x][p.y]=1;
	if(p.weapon){
...
void Grid::remove(Player p){
	c[p.x][p.y]=0;
	if(p.weapon)c[p.x+dx[p.dir]][p.y+dy[p.dir]]=0;
}
void Grid::genCoin(){
	int cnt=16;
	if(inTut)cnt=3;
...
bool ablePoint(int x,int y){
	...
	if(countKey<nKey and grid.c[x][y]==10)return 0;
	return 1;
}
bool able(Player p){
	if(!ablePoint(p.x,p.y))return 0;
	if(p.weapon and !ablePoint(p.x+dx[p.dir],p.y+dy[p.dir]))return 0;
	return 1;
}
